// ── PROFILE ──────────────────────────────────
// Depends on: app-core.js, app-payments.js
...
window.renderProfile = async function() {
  ...
  $('prof-tours').innerHTML=`
    ${myTours.length?`<div class="g2 mb16">${myTours.map(t=>{
      ...
      </div>`;}).join('')}</div>`:'<div class="empty mb16">You haven\'t joined any tournaments yet.</div>'}
    ${available.length?`
  ...
  if (thisPay?.status==='approved'){ uploadEl.innerHTML=`<span class="b b-green" style="padding:12px 20px">✓ ${tm} ${ty} payment approved${thisPay.amount?' — €'+fmt2dp(thisPay.amount):''}</span>`; return; }
  ...
  uploadEl.innerHTML=`
    ...
    <div class="fg"><label>Receipt (image or PDF)</label><input type="file" id="pm-file" accept="image/*,.pdf"></div>
